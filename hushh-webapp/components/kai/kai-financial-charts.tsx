...
export default function KaiFinancialCharts({
  quantMetrics,
  keyMetrics,
}: KaiFinancialChartsProps) {
  // Prepare Radar Data
  const radarData = [
    {
      subject: "Revenue Growth",
      value: (quantMetrics.revenue_growth_yoy || 0) * 100 * 2,
      fullMark: 100,
    }, // Scale for visualization
    {
      subject: "FCF Margin",
      value: (keyMetrics.fundamental.fcf_margin || 0) * 100 * 2,
      fullMark: 100,
    },
    {
      subject: "R&D Intensity",
      value: (keyMetrics.fundamental.rnd_intensity || 0) * 100 * 3,
      fullMark: 100,
    },
    {
      subject: "Earn Quality",
      value: (keyMetrics.fundamental.earnings_quality || 0) * 50,
      fullMark: 100,
    }, // Assuming roughly 1-2 range
    {
      subject: "Debt/Equity",
      value: Math.max(
        0,
        100 - (keyMetrics.fundamental.debt_to_equity || 0) * 20,
      ),
      fullMark: 100,
    }, // Lower is better
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="border-border/40 bg-card/40 backdrop-blur-sm">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              YoY Growth:{" "}
              {(quantMetrics.net_income_growth_yoy * 100).toFixed(1)}%
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ChartContainer
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="border-border/40 bg-card/40 backdrop-blur-sm">
          ...
          <CardContent>
            ...
            >
              <ComposedChart
                data={quantMetrics.ocf_trend_data.map((d, i) => ({
                  year: d.year,
                  ocf: d.value,
