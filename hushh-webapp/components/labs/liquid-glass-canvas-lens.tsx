...
export function LiquidGlassCanvasLens({
  ...
  useEffect(() => {
    ...
    const blurRadius =
      assets.blur > 0.01 ? Math.max(0.6, assets.blur * scale) : 0;
    ...
    if (blurRadius > 0.01) {
      ...
      if (blurCtx) {
        ...
        sourceImageData = blurCtx.getImageData(
          padding,
          padding,
          renderWidth,
          renderHeight,
    ...
    for (let y = 0; y < renderHeight; y += 1) {
      for (let x = 0; x < renderWidth; x += 1) {
        ...
        const offsetX =
          (((displacementPixels[idx] ?? 128) - 128) / 127) * scaledDisplacement;
        const offsetY =
          (((displacementPixels[idx + 1] ?? 128) - 128) / 127) *
          scaledDisplacement;
        ...
        const specularAlpha =
          ((specularPixels[idx + 3] ?? 0) / 255) *
          assets.specularOpacity *
          mix.specular;
        const shadowBias = mix.shadow * (1 - specularAlpha * 0.35);

        outputPixels[idx] = clampChannel(
          r * (1 - shadowBias) + (specularPixels[idx] ?? 0) * specularAlpha,
        );
